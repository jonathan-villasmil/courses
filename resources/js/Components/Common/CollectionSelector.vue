...
<template>
    <div class="w-full">
        ...
        <div class="flex">
            ...
            <button @click="handleAddToSelection" class="bg-indigo-500 hover:bg-indigo-700 rounded py-2 px-4 text-white ml-1">Add</button>
        </div>

        <div>
            <ul>
                <li v-for="(item) in selection"  class="bg-indigo-500 hover:bg-indigo-700 rounded py-2 px-4 text-white ml-1 mt-1 mb-1" @click="handleRemoveSelection(item.id)">
                    {{ item.name }}
